 ...
 * Automatically scans all data structures and extracts all images dynamically with usage context and metadata.
 */
export function scanSiteAssets(siteData) {
  const assetsMap = new Map();

  const addAsset = (item) => {
    const rawUrl = item.url || item.data_url;
    ...
    if (assetsMap.has(url)) {
      ...
      if (item.usage && !existing.locations.includes(item.usage)) {
  ...
  function getFilenameFromUrl(u) {
    if (u.startsWith("data:image")) return "صورة مرفوعة (Base64)";
    if (u.startsWith("/uploads/")) {
      const parts = u.split("/");
      return parts[parts.length - 1] || "صورة خادم مرفوعة.jpg";
    }
    try {
      const parts = u.split("/");
